...
/**
 * @name axchange
 * @description 将一段文本转换成繁体或简体
 * @param {string} text 目标文本
 * @param {boolean} openSimple true-转成简体 false-转成繁体
 * @returns {string} 转换后的文本
 */
export const exchange = (text: string, openSimple: boolean): string => {
    let result = '';
    const indexThesaurus: string = openSimple ? Traditional : Simplified;
    const targetThsaurus: string = openSimple ? Simplified : Traditional;
...
/**
 * @name translate
 * @description 进行某个标签内部所有文本的简繁体转换
 * @param {boolean} openSimple true-转成简体 false-转成繁体
 * @param {HTMLElement} obj 标签元素，默认是body
 */
export const translate = (openSimple: boolean, obj?: Node) => {
    const objs = (obj || document.body).childNodes;
    const stopDOM = ['BR', 'TIME', 'IMG', 'CANVAS', 'SCRIPT'];
    for (let i = 0; i < objs.length; i++) {
        const node = objs[i];
        const { nodeType, textContent, tagName } = node as HTMLElement;
        if (nodeType === 3 && /[\u4e00-\u9fa5]/g.test(textContent as string)) {
            objs[i].textContent = exchange(textContent as string, openSimple);
        } else if (nodeType === 1 && !stopDOM.includes(tagName)) {
            translate(openSimple, node);
        }
    }
};
